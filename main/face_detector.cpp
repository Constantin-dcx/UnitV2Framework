...
static void process_faceobjects(const cv::Mat& image, const std::vector<FaceObject>& faceobjects)
{
    ...
    for (size_t i = 0; i < faceobjects.size(); i++)
    {
        ...
        if(isStreamOpend())
        {
            ...
            RD_addPoint(obj.landmark[0].x, obj.landmark[0].y, "#FFCC00", IMAGE_DIV);
            RD_addPoint(obj.landmark[1].x, obj.landmark[1].y, "#FFCC00", IMAGE_DIV);
            RD_addPoint(obj.landmark[2].x, obj.landmark[2].y, "#FFCC00", IMAGE_DIV);
            RD_addPoint(obj.landmark[3].x, obj.landmark[3].y, "#FFCC00", IMAGE_DIV);
            RD_addPoint(obj.landmark[4].x, obj.landmark[4].y, "#FFCC00", IMAGE_DIV);
        #endif
...
{
    startFramework("Face Detector", 640, 480);
    ...
    while(1)
    {
        ...
        detect_retinaface(net_retinaface, mat_div, faceobjects);
        process_faceobjects(mat_div, faceobjects);
